#!/usr/bin/env python3
"""
Cloky Elite Telegram Bot v1.6 — Sprint 2 features
==================================================

Bridge: Telegram → Claude Code CLI → ANTHROPIC_BASE_URL local → Qwen llama.cpp/TurboQuant.

Cambios v1.5 → v1.6 (Sprint 2):
  S2.1  Streaming incremental a Telegram (editMessageText con rate limit).
        El usuario ve el output de Claude mientras se genera, no al final.
  S2.2  Manejo de archivos subidos por Telegram (PDF, imágenes, código).
        Bajan a workspace/uploads/ y el path se pasa al prompt.
  S2.3  Multi-proyecto con /cd <proyecto> + whitelist via ALLOWED_PROJECTS.
        Permite cambiar el cwd de Claude Code sin reiniciar servicio.

Cambios v1.4 → v1.5: Sandbox systemd configurable (strict/open).
Cambios v1.3 → v1.4: Sprint 1 + fixes auditoría.
Cambios v1.2 → v1.3: Defaults oficiales documentados.
Cambios v1.1 → v1.2: Modo autónomo.
Cambios v1.0 → v1.1: 6 patches hardening.

NO MODIFICA:
  - llama.cpp / TurboQuant (config, binarios, modelos cargados).
  - Otros bots Telegram, Agent Zero, Morgan/OpenClaw, VIPER, Taurus, BoviSense, CobraVivo.
"""
...
# v1.3: 12 mensajes con max_output_tokens=32K deja contexto sobrado.
...
def edit_message(chat_id: int, message_id: int, text: str) -> dict[str, Any]:
...
class StreamingMessage:
    ...
    def append(self, chunk: str, force: bool = False) -> None:
        ...
        with self.lock:
            ...
            if len(self.current_text) > STREAM_PREVIEW_LIMIT:
                # Cortar en límite de línea cuando sea posible
                ...
                # Editar el mensaje actual con head completo
                if self.message_ids:
                    edit_message(self.chat_id, self.message_ids[-1], head)
                # Abrir nuevo mensaje con tail (sin notification)
    ...
    def finalize(self, full_text: str | None = None) -> None:
        ...
        del último mensaje con la versión final (con header completo etc.).
        """
        with self.lock:
            if full_text is not None:
                self.current_text = full_text
            if self.message_ids:
                # Editar último mensaje con contenido final
                final = self.current_text[-STREAM_PREVIEW_LIMIT:] if len(self.current_text) > STREAM_PREVIEW_LIMIT else self.current_text
                edit_message(self.chat_id, self.message_ids[-1], final or "(sin contenido)")
...
def run_claude_task(user_id: str, chat_id: int, message_id: int, user_text: str) -> None:
    ...
    # Construir command con flag de auto-aprobación si está habilitado.
    command = [CLAUDE_BIN]
    ...
    log(f"START_TASK user={user_id} auto_approve={CLAUDE_AUTO_APPROVE} cwd={cwd}")
    ...
    try:
        ...
        if streamer is not None and streamer.message_ids:
            ...
            for extra in chunks[1:]:
                send_message(chat_id, extra, notify=False)
            ...
            send_message(chat_id, final_text, reply_to_message_id=message_id, notify=notify)
...
def handle_command(user_id: str, chat_id: int, message_id: int, text: str) -> bool:
    ...
    if command in {"/start", "/help"}:
        ...
        send_message(
            chat_id,
            (
                f"Bot Claude Code v{VERSION} — {auto_msg}.\n"
                f"{sandbox_msg}\n\n"
                "Operación:\n"
                "/status   - estado de la tarea activa\n"
                "/cancel   - cancelar tarea activa (preserva sesión)\n"
                "/stop     - matar tarea activa (más agresivo)\n"
                "/clear    - limpiar historial de contexto\n\n"
                "Workspace:\n"
                "/cwd      - mostrar cwd actual\n"
                "/pwd      - alias de /cwd\n"
                "/cd <p>   - cambiar a proyecto de la whitelist\n"
                "/projects - listar proyectos disponibles\n\n"
                "Diagnóstico:\n"
                "/health   - probar backend Qwen end-to-end\n"
                "/config   - tokens, timeouts, modo sandbox, streaming\n"
                "/version  - versión del bot\n"
                "/uptime   - tiempo desde el último reinicio\n"
                "/tasks    - tareas activas en curso\n"
                "/stats    - métricas acumuladas\n\n"
                "Archivos (S2.2):\n"
                "Mandá fotos, docs, audio o video y se descargan a workspace/uploads/.\n"
                "El path queda visible en el prompt para que Claude los procese.\n\n"
                "Texto libre → tarea para Claude Code.\n"
                "Streaming activo: ves el output mientras se genera.\n\n"
                "Bloqueos siempre activos: rm -rf /, mkfs, dd a discos,\n"
                "fork bombs, shutdown, chmod/chown -R en /."
            ),
    ...
    if command == "/config":
        ...
        send_message(
            ...
            (
                ...
                f"context_messages:  {MAX_CONTEXT_MESSAGES}\n"
                f"auto_approve:      {CLAUDE_AUTO_APPROVE}\n"
    ...
    if command == "/cancel":
        # v1.4 S1.2: cancelación SUAVE con SIGINT
        # Claude Code intercepta SIGINT como "cancel current operation".
        # Si la tool actual respeta SIGINT, se aborta sin matar el proceso.
        # Si después de 5s no termina, fallback a SIGTERM como /stop.
        with task_lock:
            task = running_tasks.get(user_id)
        if not isinstance(task, RunningTask):
            if task == "PENDING":
                send_message(
                    chat_id,
                    "Tarea en PENDING. Esperá 1-2s y reintentá.",
                    reply_to_message_id=message_id,
                )
            else:
    ...
    if command == "/stop":
        # v1.4 S1.2: matar el proceso completo (SIGTERM → escalada a SIGKILL si necesario)
        with task_lock:
            task = running_tasks.get(user_id)
        if not isinstance(task, RunningTask):
            if task == "PENDING":
